...
/*
 ...
 Service 객체는 Controller와 Model사이에서 중재역할을 함.
 */
@Service
public class ListCommand implements BbsCommandImpl{
	...
	public void execute(Model model) {
		...
		/*
		 컨트롤러에서 넘겨준 파라미터를 asMap() 메소드를 통해 Map컬렉션으로
		 반환. 그리고 request객체를 형변환하여 가져온다.
		 */
		...
		//현재페이지번호 첫 진입이라면 무조건 1페이지
		int nowPage = req.getParameter("nowPage")==null ? 1:
			Integer.parseInt(req.getParameter("nowPage"));
		
		//리스트에 출력할 게시물의 시작/종료 구간(select절의 
		//between에 사용)
		...
		for(SpringBbsDTO row : listRows) {
			...
			//답변글에 대한 리스트처리
			String reSpace = "";
			//해당게시물의 indent가 0보다 크면 (답변글이라면) 
			if(row.getBindent()>0) {
				//indent의 크기만큼 공백(&nbsp;)를 추가.
		...
		String pagingImg = PagingUtil.pagingImg(totalRecordCount,
				...
				req.getContextPath() + "/board/list.do?"
				+addQueryString);
